...
Needs to be fed list of time index for atc + date in format year-month-date
Example run: imbalance, atc = GurobiModel(date='2019-01-01', atc=df_atc, time=time_idx_list).run()
"""
class GurobiModel:

    def __init__(self, date, atc_pos, atc_neg, time, name=''):
        ...
        self.date = date
        self.name = name
        self.path = f'C:\\Users\\hnordstr\\OneDrive - KTH\\box_files\\KTH\\Papers&Projects\\DynamicFRR\\Normal Imbalances\\Scenario' #Minute normal imbalances
        ...
        self.num_scenarios = int(self.time.__len__() / 24)
    ...
    def setup_problem(self):
        ...
        self.set_INTERVALS = list(range(self.time.__len__() * 60 - 1))
        ...
        for l in self.set_ACINDX:
            self.param_ATC_POS[l] = [self.atc_pos[l][i] for i in range(self.atc_pos.__len__()) for x in range(60)]
            self.param_ATC_NEG[l] = [self.atc_neg[l][i] for i in range(self.atc_neg.__len__()) for x in range(60)]
        for a in self.areas:
            sto_list = []
            det_list = []
            ...
            for t in self.time:
                det_list.extend(self.deterministic[a][datetime.strftime(t, '%Y-%m-%d %H:%M:%S+00:00'):
                     datetime.strftime(t + timedelta(minutes=59), '%Y-%m-%d %H:%M:%S+00:00')].values)
