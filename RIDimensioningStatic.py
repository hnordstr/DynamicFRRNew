...
class RIDimensioning:
    ...
    class MIP:
        ...
        def setup_problem(self):
            ...
            up_down_selector = ['Up', 'Down']
            up_down_list = [random.choice(up_down_selector) for n in self.SET_SCENARIOS]
            for i in self.SET_SCENARIOS:
                area_choice = random.choice(self.SET_AREAS)
                for a in self.SET_AREAS:
                    if a == area_choice and up_down_list[i] == 'Up':
                        self.PARAM_IMBALANCES_DIST[a][i] = - self.RI_up[a]
                    elif a == area_choice and up_down_list[i] == 'Down':
                        self.PARAM_IMBALANCES_DIST[a][i] = self.RI_down[a]
                    else:
                        self.PARAM_IMBALANCES_DIST[a][i] = 0
